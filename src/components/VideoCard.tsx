...
import { isImageMediaUrl } from '../lib/videos'
...
export default function VideoCard({ video, onOpen }: { video: VideoItem; onOpen: (video: VideoItem) => void; index?: number }) {
  ...
  const initialThumb = video.thumbnailUrl || (isImage ? video.videoUrl : 'thumbnails/fallback.svg')
  const [src, setSrc] = useState(publicUrl(initialThumb || 'thumbnails/fallback.svg'))
  const difficulty = getDifficultyMeta(video.difficulty)
  const aspectRatio = normalizeAspectRatio(video.aspectRatio)
  const hasDisplayStyle = video.style && video.style !== 'altro' && video.style !== 'gfx'
  ...
  return (
    <motion.article initial={false} className="group min-w-0">
      <button onClick={() => onOpen(video)} className="block w-full text-left focus-ring" aria-label={`Open ${video.title}`}>
        <div className={`relative rounded-[1.45rem] p-[2px] ${frameClass}`}>
          <div className="relative aspect-video overflow-hidden rounded-[1.32rem] bg-panel shadow-2xl shadow-black/25" style={aspectRatio ? { aspectRatio } : undefined}>
            <img src={src} onError={() => setSrc(publicUrl('thumbnails/fallback.svg'))} alt={`${video.title}`} loading="lazy" decoding="async" className="h-full w-full object-cover transition duration-700 ease-out group-hover:scale-[1.045] group-hover:saturate-125" />
            ...
            <div className="absolute inset-x-0 top-0 flex items-start gap-3 p-4">
              {hasDisplayStyle ? (
                ...
                <span className="rounded-full border border-white/15 bg-black/45 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[.14em] text-white/70 backdrop-blur-md">GFX</span>
              ) : null}
            ...
            <span className="absolute bottom-4 right-4 grid h-12 w-12 place-items-center rounded-full bg-white text-ink shadow-xl transition-all duration-300 group-hover:scale-110 group-hover:bg-flame group-hover:text-white">
              {isImage ? <Eye size={17} /> : <Play size={16} fill="currentColor" className="translate-x-px" />}
            </span>
          </div>
        </div>
      </button>
