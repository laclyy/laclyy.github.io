...
export default function App() {
  ...
  return <div className="min-h-screen overflow-clip bg-ink text-white"><div className="noise pointer-events-none fixed inset-0 z-[100] opacity-[.035]" /><Navbar profile={profile} /><AnimatePresence mode="wait"><motion.div key={location.pathname} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .25 }}><Routes location={location}><Route path="/" element={<HomePage profile={profile} socials={socials} />} /><Route path="/videos" element={<VideosPage videos={videos} loading={loading} onOpen={setSelected} />} /><Route path="/contact" element={<ContactPage profile={profile} socials={socials} />} /><Route path="*" element={<HomePage profile={profile} socials={socials} />} /></Routes></motion.div></AnimatePresence><Footer profile={profile} socials={socials} /><VideoModal video={selected} onClose={closeModal} /></div>
}
