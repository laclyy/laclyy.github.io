...
export default function HomePage({ profile, videos, onOpen }: { profile: Profile; videos: VideoItem[]; onOpen: (video: VideoItem) => void }) {
  return <>
    <Hero profile={profile} />
    <section className="section-space border-y border-white/[.06] bg-white/[.012]"><div className="shell grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:gap-20"><div><div className="eyebrow"><span className="h-px w-7 bg-flame" />{profile.role}</div><p className="mt-5 font-display text-2xl font-medium leading-tight tracking-tight text-white/88 md:text-3xl">{profile.shortDescription}</p>{profile.location && <p className="mt-5 font-mono text-[10px] uppercase tracking-[.15em] text-white/28">Based in {profile.location}</p>}</div><div className="grid gap-6 text-base leading-7 text-white/50 sm:grid-cols-2"><p>{profile.longDescription}</p><p>{profile.styleDescription}</p></div></div></section>
