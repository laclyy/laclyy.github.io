...
export default function VideosPage({ videos, loading, onOpen }: { videos: VideoItem[]; loading: boolean; onOpen: (video: VideoItem) => void }) {
  ...
  return <main className="pb-28 pt-32 md:pb-36 md:pt-40">
    ...
    <section className="shell mt-14 md:mt-20"><DifficultyLegend /><div className="mt-8 md:mt-10"><VideoFilters filters={filters} onChange={setFilters} count={filtered.length} videos={videos} /></div><div className="mt-12"><VideoGrid videos={filtered} loading={loading} onOpen={onOpen} /></div></section>
  </main>
}
